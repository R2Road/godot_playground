...
func _ready():
	...
	pam.add_lf()
	pam.add_mover( timer_basic.scene_name(),		Key.KEY_Q, 		"res://2d/etc/etc_timer_basic.tscn" )
	pam.add_mover( screen_exit.scene_name(),		Key.KEY_W, 		"res://2d/etc/etc_screen_exit.tscn" )
